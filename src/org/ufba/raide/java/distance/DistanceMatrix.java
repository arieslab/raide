...
import org.ufba.raide.java.ast.association.Association;
import org.ufba.raide.java.ast.util.math.Cluster;
import org.ufba.raide.java.ast.util.math.Clustering;
...
public class DistanceMatrix {
    ...
    private List<AddExplanationCandidateRefactoring> identifyConceptualBindings(MyMethod method, Set<String> targetClasses) {
    	List<AddExplanationCandidateRefactoring> candidateRefactoringList = new ArrayList<AddExplanationCandidateRefactoring>();
	    					...
	    					if(methodInvocation.getOriginClassName().equals(targetClass)) {
	    						...
	    						if(parameterIsPassedAsArgument) {
	    							...
	    							if(containerFieldIsAccessed) {
	    								...
	    								AddExplanationCandidateRefactoring candidate = new AddExplanationCandidateRefactoring(system,mySourceClass,myTargetClass,method);
    ...
    public List<AddExplanationCandidateRefactoring> getMoveMethodCandidateRefactoringsByAccess(Set<String> classNamesToBeExamined, IProgressMonitor monitor) {
    	List<AddExplanationCandidateRefactoring> candidateRefactoringList = new ArrayList<AddExplanationCandidateRefactoring>();
    	if(monitor != null)
    	...
    	for(int i=0; i<entityList.size(); i++) {
    		...
    		if(entity instanceof MyMethod) {
    			...
    			if(classNamesToBeExamined.contains(sourceClass)) {
    				...
    				List<AddExplanationCandidateRefactoring> conceptuallyBoundRefactorings = identifyConceptualBindings(method, accessMap.keySet());
